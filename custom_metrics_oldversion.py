...
customAlgorithmInfo = [{"name": "insitu_at", #Human readable name, this can be set to anything and is only used as a label
                       "outputVar": "AT", #DIC or AT
                       "matchupVariableName": "insitu_ta_mean", #netCDF variable name of the model output (algorithm prediction)
                      ...
                      {"name": "insitu_dic", #Human readable name, this can be set to anything and is only used as a label
                       "outputVar": "DIC", #DIC or AT
                       "matchupVariableName": "insitu_dic_mean", #netCDF variable name of the model output (algorithm prediction)
...
sssDatasetName = "SST-ESACCI"; #This is the matchup database variable name corresponding to the SSS dataset used as input for the Ethz data
...
if runPairedMetrics == True:
    ...
    for region in regions:
        ...
        customAlgorithms = [cai for cai in customAlgorithmInfo if cai["outputVar"] in ["AT", "DIC", "region_pH_mean", "region_pco2w_mean"]];
...
if runBasicMetrics == True:
    ...
    for customAlgo in customAlgorithmInfo:
        ...
        ##### TODO:
        ##### Missing: combined uncertainty, matchupRMSD, matchupBias?
        customAlgoData = utilities.read_matchup_cols(settings["matchupDatasetTemplate"], colsToExtract, years); #returns data frame containing data from the matchup database for each variable in 'col
        ###Subset to remove where model data is NaN
        customAlgoData = customAlgoData.loc[np.isfinite(customAlgoData[customAlgo["matchupVariableName"]])]; #remove where there is no model predictions
        customAlgoData = customAlgoData.loc[np.isfinite(customAlgoData[customAlgo["outputVar"]])]; #remove where there is no reference outputVar data
