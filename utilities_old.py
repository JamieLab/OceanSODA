            ...
            try:
                if col == "date": #some variables have different names in the matchup. This is hacky, but manually matches the commonName and matchupdatabase names. This is probably ok, as these are unlikely to change. Should really look into the variable:matchup mapping in the global settings file
                    ...
                    try: #If there is a missing data value, filter out missing values and replace with nans
                        ...
                        pass;
                elif col == "region_pH_mean":
                    df[col] = matchupNC.variables["insitu_ph_mean"][:];
                    try: #If there is a missing data value, filter out missing values and replace with nans
                        missingValue = matchupNC.variables["insitu_ph_mean"]._FillValue;
                        if np.isnan(missingValue) == False:
                            df.loc[df["region_pH_mean"]==missingValue, "region_pH_mean"] = np.nan;
                    except:
                        pass;
                elif col == "region_pco2w_mean":
                    df[col] = matchupNC.variables["insitu_pco2w_mean"][:];
                    try: #If there is a missing data value, filter out missing values and replace with nans
                        missingValue = matchupNC.variables["insitu_pco2w_mean"]._FillValue;
                        if np.isnan(missingValue) == False:
                            df.loc[df["region_pco2w_mean"]==missingValue, "region_pco2w_mean"] = np.nan;
                    except:
                        pass;        
