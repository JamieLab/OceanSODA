...
for region in regions:
    ...
    counter_hov=0;
        #### figure dimensions
    fig42 = plt.figure(figsize=(25,24))
    gs = fig42.add_gridspec(6, 1,hspace=0.28)
    fig42.tight_layout(pad=3.0)
    ...
    for plot_var in hovmoll_vars:
        ...
        if region == "oceansoda_amazon_plume":
            ...
            if plot_var == "DIC":
                ...
                maxvar = 8.4;#np.nanmax(data);
                minvar = np.nanmin(data);
                data_masked=np.ma.masked_less(data, 8.4)
                data=np.ma.masked_greater(data, 8.4)
            elif plot_var == "pCO2":
                maxvar = np.nanmax(data);
                minvar = 200;#np.nanmin(data); 
                data_masked=np.ma.masked_greater(data, 200)
                data=np.ma.masked_less(data, 200)
            elif plot_var == "omega_aragonite":
                maxvar = 5;#np.nanmax(data);
                minvar = np.nanmin(data);
                data_masked=np.ma.masked_less(data, 5)
                data=np.ma.masked_greater(data, 5)
            elif plot_var == "omega_calcite":                 
                maxvar = 7;#np.nanmax(data);
                minvar = np.nanmin(data); 
                data_masked=np.ma.masked_less(data, 7)
                data=np.ma.masked_greater(data, 7)
            else:
                pass
        elif region == "oceansoda_mediterranean":
            if plot_var == "DIC":
        ...
        if region == "oceansoda_congo" and plot_var == "pH_free_scale":
            ...
            contPlot1 = f3_ax.pcolor(dates,lons,  data, vmin=minvar, vmax=maxvar, cmap=cmap_touse);
            f3_ax.set_ylabel("Longitude \n ($^\circ$E)", fontsize=36);
            contPlot2 = f3_ax.pcolor(dates,lons,  data_masked, vmin=minvar, vmax=maxvar, cmap=newcmp);
            ...
            contPlot1 = f3_ax.pcolor(dates,lats,  data,  cmap=cmap_touse);
        ...
        if plot_var == "DIC":
            ...
            cb.set_label("pCO$_{2}$ $(\mu atm)$", fontsize=36);
        elif plot_var == "omega_aragonite":
    ...
    fig42.savefig(path.join("os_plots\{0}_seasonal".format(region), "_hov_multivar_all_vars_eps.eps"), format='eps');
    ...
    ax41.set_ylabel("pCO$_{2}$ $(\mu atm)$", fontsize=36);
    ...
    plt.savefig("os_plots\\fig4_carbonate_timeseries_{0}_eps.eps".format(region), format='eps');    
    ...
    for subplot_loop_no in datetoplotall:
        ...
        if region == "oceansoda_amazon_plume":
            if plot_var == "DIC":
                ...
                minvar = 1600;  #;np.nanmin(dataall_subplots);
                data_masked=np.ma.masked_greater(data, 1600)
                data=np.ma.masked_less(data, 1600)
            elif plot_var == "TA":
                maxvar = np.nanmax(dataall_subplots);#2400;
                minvar = 1800;#np.nanmin(dataall_subplots);;
            ...
            if plot_var == "DIC":
                ...
                minvar = 1750;#np.nanmin(dataall_subplots); 
                data_masked=np.ma.masked_greater(data, 1750)
                data=np.ma.masked_less(data, 1750)
            elif plot_var == "TA":
                maxvar = np.nanmax(dataall_subplots);
        ...
        if plot_var == "DIC" and region == "oceansoda_amazon_plume":
            ...
            #this is a plot of the masked region out of range
            contPlot2 = f3_ax.pcolor(lons, lats, data_masked,vmin=minvar, vmax=maxvar,  cmap=newcmp);
        elif plot_var == "DIC" and region == "oceansoda_congo":
            #data plot
            contPlot1 = f3_ax.pcolor(lons, lats, data, vmin=minvar, vmax=maxvar, cmap=cmap_touse);
            #this is a plot of the masked region out of range
            contPlot2 = f3_ax.pcolor(lons, lats, data_masked,vmin=minvar, vmax=maxvar,  cmap=newcmp);
    ...
    fig6.savefig(path.join("os_plots\{0}_seasonal".format(region), "{0}_monthly_averages_eps.eps".format(plot_var)), format='eps');
    ...
    for subplot_loop_no in datetoplotall:
        ...
        if region == "oceansoda_amazon_plume":
            if plot_var == "DIC":
                ...
                maxvar = np.nanmax(dataall_subplots);#2400;
                minvar = 2000;#np.nanmin(dataall_subplots);
...
unqiuealgos_at_amazon2=("(Astor et al. 2017)","(Astor et al. 2017)","(Astor et al. 2017)","(Astor et al. 2017)","(Brewer et al. 1995)","(Cai et al. 2010)","(Lefèvre et al. 2010)","(Millero et al. 1998)","(Ternon et al. 2000)","(Cooley et al. 2006)")
...
unqiuealgos_dic_amazon2=("(Brewer et al. 1995)","(Lee et al. 2000)","(Lefèvre et al. 2010)","(Lefèvre et al. 2017)","(Ternon et al. 2000)","(Cooley et al. 2006)")
...
unqiuealgos_at_Congo2=("(Brewer et al. 1995)","(Goyet et al. 1998)","(Lee et al. 2006)","(Takahashi et al. 2014)");
...
unqiuealgos_dic_CONGO2=("(Bakker et al. 1999)","(Bakker et al. 1999)","(Bakker et al. 1999)","(Brewer et al. 1995)","(Lee et al. 2000)","(Vangriesheim et al. 2009)","(Vangriesheim et al. 2009)")   
...
fig1.savefig("os_plots\\Fig1_RMSDe_eps.eps", format='eps');
...
f1_ax3 = fig1.add_subplot(gs[20:26, 10:50])
...
f1_ax2 = fig1.add_subplot(gs[30:39, 10:50])
...
f1_ax4 = fig1.add_subplot(gs[43:51, 10:50])
        ...
        for subplot_loop_no in datetoplotall:
            ...
            if region == "oceansoda_amazon_plume":
                if plot_var == "DIC":
                    ...
                    minvar = np.nanmin(data); 
                pass
            fig8, ax = plt.subplots(figsize=(20, 12))
            f8_ax = plt.axes(projection=ccrs.PlateCarree())           
            gl = f8_ax.gridlines(crs=ccrs.PlateCarree(), draw_labels=True, linewidth=1, color='gray', alpha=0.5, linestyle='--');
            ...
            f8_ax.set_xlabel("Longitude($^\circ$)", fontsize=28);
            f8_ax.set_ylabel("Latitude($^\circ$)", fontsize=28);
            ...
            f8_ax.text(-0.08, 0.45, 'Latitude $(^{\circ})$', va='bottom', ha='center',
                rotation='vertical', rotation_mode='anchor',fontsize=28,
                transform=f8_ax.transAxes)
            f8_ax.text(0.5, -0.11, 'Longitude $(^{\circ})$', va='bottom', ha='center',
                rotation='horizontal', rotation_mode='anchor',fontsize=28,
                transform=f8_ax.transAxes)
            
            if plot_var == "DIC":
                cb.set_label("DIC ($\mu mol \ kg^{-1}$)", fontsize=28);
                #f8_ax.set_title("DIC for {0} {1}".format(date.year, format(date.month, "02d")), fontsize=24);  
            elif plot_var == "SSS":
                cb.set_label("Salinity (PSU)", fontsize=28);
                #f8_ax.set_title("Salinity for {0} {1}".format(date.year, format(date.month, "02d")), fontsize=24);
            elif plot_var == "SST":
                cb.set_label("SST ($^\circ$ C)", fontsize=28);
                #f8_ax.set_title("SST for {0} {1}".format(date.year, format(date.month, "02d")), fontsize=24);
            elif plot_var == "TA":
                cb.set_label("TA ($\mu mol \ kg^{-1}$)", fontsize=28);
            ...
            fig8.savefig(path.join("os_plots\{0}_seasonal".format(region,plot_var), "{0}singlemonth_jpg.jpg".format(plot_var)));
            fig8.savefig(path.join("os_plots\{0}_seasonal".format(region,plot_var), "{0}singlemonth_eps.eps".format(plot_var)), format='eps');
